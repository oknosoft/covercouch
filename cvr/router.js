  ...
  var actors = {
    ...
    db: function (req, res, next) {
      ...
      function _restrict() {
        ...
        if(!dbv.restricted || !dbv._restrict[req.method]) {
          _isLong();
        }
        else {
          var acl = null,
          ...
          if(!allow) {
            _fail(req, res, {error: 'forbidden', reason: 'Method restricted.'}, 403);
          }
          else {
            _isLong();
          }
    ...
    attachment: function (req, res, next) {
      ...
      if(!acl || !acl.a || u.admin || acl.a.every(({n}) => n !== fname) ||
          acl.a.some(({n, r}) => n === fname && (r === '_anonymous' || u.roles.includes(r)))) {
        next();
      }
      else {
        _fail(req, res, {error: "forbidden", reason: `ACL: доступ к вложению ${fname}, запрещен пользователю ${u.name}`}, 403);
      }
    },
    ...
    bulk: function (req, res) {
      ...
      if(isA(d) && d.length) {
        ...
        if(atomic && errs) {
          ...
          cvr.Request(p).then(function (data) {
            var a, ctr = 0;
            try {
      ...
      cvr.request(p)
        ...
        .pipe(es.map(function (data, done) {
          ...
          if(!data.length) {
            ...
            if(json) {
              if(ctr == 1 || data.to(11) === '"last_seq":') {
                ok = true;
              }
              else if(data.to(2) === '],') ok = true;
            }
            else if(m == 'ev' && data.to(3) === 'id:') {
              if(seq[data.from(3).trim()]) {
                ok = true;
              }
              else {
                ok = false;
    ...
    list: function (req, res) {
      ...
      function _list(err) {
        if(!err) {
          if(isReduce) {
            viewResult = cvr.Sandbox.reduce(req, rows);
          }
          else {
          ...
          _fail(req, res, {error: 'error', reason: err.reason}, err.statusCode);
        }
      }
    },
          ...
          req.pipe(cvr.request(p))
            ...
            .pipe(es.mapSync(function (data) {
              ...
              if(!data.length || end || data.last() === '[') {
                ...
                if(id) {
                  ok = !!cvr.ACL.doc(req.session, req.params.db, id)._r;
                  if(ok && jsopts.attachments) {
                    const acl = cvr.db[req.params.db].acl[id];
                    const u = cvr.user[req.session.user];
                    //TODO: list attachments acl
                    if(acl && acl.a && !u.admin && !acl.a.some(({r}) => r === '_anonymous' || u.roles.includes(r))) {
                      ok = false;
                    }
                  }
                }
